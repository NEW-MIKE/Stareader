 ...
 */
package com.kaya.stareader.logic;
...
import com.kaya.stareader.utils.Constant;
...
public class CacheManager {
    ...
    public List<BookLists.BookListsBean> getCollectionList() {
        List<BookLists.BookListsBean> list = (ArrayList<BookLists.BookListsBean>) ACache.get(
                StareaderApplication.getsInstance()).getAsObject(getCollectionKey());
        ...
        for (BookLists.BookListsBean bean : list) {
            if (bean != null) {
                if (TextUtils.equals(bean._id, bookListId)) {
                    list.remove(bean);
                    ACache.get(StareaderApplication.getsInstance()).put(getCollectionKey(), (Serializable) list);
                    break;
                }
            }
        }
    ...
    public synchronized void clearCache(boolean clearReadPos, boolean clearCollect) {
        try {
            ...
            if (clearReadPos) {
                ...
                StareaderApplication.getsInstance().getmAppPreferencesHelper().removeAll();
